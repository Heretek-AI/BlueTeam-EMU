...
The system SHALL provide a faceted log search that returns log events
...
Search SHALL run client-side against the sql.js database via the
function `searchLogs({ operationId, filters, limit })` exported from
`apps/web/src/lib/client/siem.ts`. There SHALL be no `/api/console/siem/search`
endpoint.

#### Scenario: Matching query returns events
- **WHEN** the analyst submits `{ "filters": [{"field":"host",
  "value":"WS01"}], "limit": 50 }`
- **THEN** the function returns up to 50 events for host `WS01`
  ordered by timestamp ascending

#### Scenario: Limit above hard ceiling is clamped
- **WHEN** the analyst submits `limit: 5000`
- **THEN** the function clamps the limit to `1000` and returns at most
  `1000` events

#### Scenario: Non-matching query returns empty list
- **WHEN** the analyst submits a query that matches no events
- **THEN** the function returns `{ events: [], total: 0 }`
...
record in the sql.js database. Each saved search SHALL store a name,
the structured query, and the timestamp of the most recent run. The
system SHALL return only searches belonging to the local user and
SHALL enforce a maximum of `50` saved searches per user.

Saved-search CRUD SHALL be exposed as client-side functions in
`apps/web/src/lib/client/siem.ts` — `listSavedSearches()`,
`createSavedSearch(name, query)`, `deleteSavedSearch(id)`. There
SHALL be no `/api/console/siem/saved` endpoint.

#### Scenario: Saving a new search persists it
- **WHEN** the analyst saves a search with a name and query
- **THEN** the search is persisted to IndexedDB and appears in the
  analyst's saved search list
