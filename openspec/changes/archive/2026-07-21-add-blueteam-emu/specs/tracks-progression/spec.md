...
The system SHALL load tracks from `content/tracks.json`, each track
declaring an ordered list of operation ids and a `gating.threshold`
integer in `0..100`. The system SHALL display tracks in the UI with
their declared ordering and SHALL allow the analyst to enroll in any
loaded track.
...
system SHALL persist the completion timestamp and SHALL expose a
"completion badge" on the dashboard and tracks list pages.
...
the local user record and SHALL reload them on every read so the
dashboard renders correctly across application restarts.
