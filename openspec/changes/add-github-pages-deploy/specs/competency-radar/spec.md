...
Aggregation inputs SHALL be read client-side from the sql.js
database via `aggregateRadar(scope)` in
`apps/web/src/lib/client/radar.ts`. There SHALL be no server-side
...
against its scope in the sql.js database and SHALL recompute the
aggregate whenever an underlying run's submissions change.
